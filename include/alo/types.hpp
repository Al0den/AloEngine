...
// Object-oriented Board with method helpers; fields remain public to minimize churn
namespace alo {
struct Board {
    // data
    int pieces[120];
    ...
    int toFEN(char *buf, size_t bufSize) const;

    // Accessors (incremental encapsulation)
    int getSide() const { return side; }
    void setSide(int s) { side = s; }

    int getEnPassant() const { return enPas; }
    void setEnPassant(int sq) { enPas = sq; }

    int getCastlePerm() const { return castlePerm; }
    void setCastlePerm(int cp) { castlePerm = cp; }

    U64 getPosKey() const { return posKey; }
    void setPosKey(U64 k) { posKey = k; }

    int getPly() const { return ply; }
    void setPly(int v) { ply = v; }
    int getHisPly() const { return hisPly; }
    void setHisPly(int v) { hisPly = v; }

    int getFiftyMove() const { return fiftyMove; }
    void setFiftyMove(int v) { fiftyMove = v; }

    int getKingSq(int c) const { return kingSq[c]; }
    void setKingSq(int c, int sq) { kingSq[c] = sq; }

    int getPiece(int sq) const { return pieces[sq]; }
    void setPiece(int sq, int p) { pieces[sq] = p; }
};
...
// (dedup) FilesBrd/RanksBrd declared once above
...
extern void ReInitHashTable(S_HASHTABLE *table, int megabytes);
